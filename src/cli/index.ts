...
program
  ...
  .option('-p, --port <number>', 'Port to run the server on (overrides saved port)')
  ...
  .action(async (options) => {
    ...
    const port = options.port ? parseInt(options.port, 10) : (savedPort ?? DEFAULT_PORT);
    ...
    if (!fs.existsSync(testResultsDir)) {
      console.log(
        chalk.yellow(`⚠️  No test results directory found at: ${testResultsDir}\n`),
      );
      console.log(chalk.gray('   Run your tests first: npx playwright test\n'));
    } else if (options.resultsDir) {
      console.log(chalk.gray(`Using custom results directory: ${testResultsDir}\n`));
    }
  ...
  .action((portNumber, options) => {
    ...
    if (!portNumber) {
      ...
      if (currentPort) {
        ...
        console.log(chalk.gray(`No custom port set. Using default: ${DEFAULT_PORT}`));
      }
      return;
    }
...
program
  ...
  .action((options) => {
    ...
    console.log(chalk.gray('1. Run your Playwright tests to generate screenshots'));
    console.log(chalk.gray('2. Run `npx stagemask review` to open the mask editor\n'));
  });
...
program
  ...
  .action((screenshot, options) => {
    ...
    console.log(chalk.green(`✓ Cleared ${existing.length} mask(s) from "${screenshot}"`));
  });
